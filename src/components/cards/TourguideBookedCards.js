...
import {formatDate, formatDateTime, formatToUSD} from '../../Library/Common';

const TourguideBookedCard = ({onPress, props}) => {
  return (
    <TouchableOpacity
      onPress={onPress}
      activeOpacity={0.9}
      ...
      <Text numberOfLines={2} style={styles.tourguideDescription}>
        {props?.offeringData ? props?.offeringData?.title : props?.title}
      </Text>
      ...
      <View style={styles.tourguideProfile2}>
        <Ionicons name="calendar-outline" size={15} style={{marginRight: 6}} />
        <Text>{formatDateTime(props?.date)}</Text>
      </View>
      <View
        style={{
          flexDirection: 'row',
...
export default TourguideBookedCard;
